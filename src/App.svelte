<script lang="ts">
    ...
    let errorMsg = $state("");
    let gpsError = $state("");
    let currentLocation = $state<{ lng: number; lat: number } | null>(
        loadLocation(),
    );
    ...
    const minutelyMaxPrecip = $derived(
        minutelyRain
            ? Math.max(0.1, ...minutelyRain.items.map((item) => item.precip))
            : 0.1,
    );
    ...
    async function requestCurrentLocation(): Promise<{
        lng: number;
        lat: number;
    }> {
    ...
    async function getRainAlertLocation(): Promise<{
        lng: number;
        lat: number;
    }> {
    ...
    async function syncRainAlertLocation(
        lng: number,
        lat: number,
    ): Promise<void> {
...
<main class="app-shell">
            ...
            <div class="header-actions">
                {#if rainAlertSupported}
                    <button
                        ...
                        aria-label={rainAlertOn
                            ? "关闭降雨提醒"
                            : "开启降雨提醒"}
                        disabled={rainAlertBusy}
                        onclick={toggleRainAlert}>🔔</button
                    >
                {/if}
                <button
                    type="button"
                ...
                <button
                    ...
                    aria-label="重新定位"
                    disabled={isLoading}
                    onclick={relocate}>{isLoading ? "···" : "📍"}</button
                >
            </div>
        </header>
    ...
    {#if minutelyOpen}
        ...
        ></button>
        <dialog class="rain-modal" aria-labelledby="minutely-title" open>
            <header class="rain-modal__header">
                ...
                <button
                    ...
                    aria-label="关闭短时降水弹框"
                    disabled={minutelyLoading}
                    onclick={closeMinutelyRain}>×</button
                >
            </header>
                ...
                <div class="rain-loading" aria-live="polite">
                    ...
                    <div class="chart-skeleton" aria-hidden="true">
                        {#each Array.from({ length: 24 }) as _, index}
                            <span style={`height: ${18 + ((index * 11) % 44)}%`}
                            ></span>
                        {/each}
                    </div>
                </div>
            {:else if minutelyError}
                <div class="rain-error">
                    <p>{minutelyError}</p>
                    <button type="button" onclick={openMinutelyRain}
                        >重试</button
                    >
                </div>
            {:else if minutelyRain}
                <p class="rain-summary">
                    {minutelyRain.summary || "暂无短时降水描述"}
                </p>
                <div class="rain-chart" aria-label="未来两小时降水量柱状图">
                    {#each minutelyRain.items as item, index}
                        <div
                            class="rain-bar"
                            title={`${formatMinutelyTime(item.fxTime)} ${item.precip.toFixed(2)}mm`}
                        >
                            <span
                            ...
                            ></span>
                            {#if index % 6 === 0}
                                <span class="rain-bar__time"
                                    >{formatMinutelyTime(item.fxTime)}</span
                                >
                            {/if}
                        </div>
                    {/each}
                ...
                <div class="rain-scale">
                    ...
                    <span
                        >峰值 {Math.max(
                            ...minutelyRain.items.map((item) => item.precip),
                        ).toFixed(2)}mm</span
                    >
                </div>
            {/if}
...
<style>
    .app-shell {
        position: relative;
        min-height: 100dvh;
        padding: calc(env(safe-area-inset-top, 0px) + clamp(16px, 3vw, 28px))
    ...
    .rain-bar__fill.wet {
        ...
        box-shadow: 0 -2px 10px rgb(56 189 248 / 20%);
    }
